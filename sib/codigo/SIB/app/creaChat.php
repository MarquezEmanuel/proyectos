...
/* Nombre del creador del chat */

$busca = "SELECT nombre FROM [usuario] WHERE legajo = '$legajo'";
$buscado = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $busca);
$row = sqlsrv_fetch_array($buscado);
$nombre = $row['nombre'];
...
$sql = "INSERT INTO chats VALUES ('$titulo', '$nombre', '$fecha', '$legajo')";
$resultado = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sql);
$query = "SELECT SCOPE_IDENTITY() id";
$lastId = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $query);
$row = sqlsrv_fetch_array($lastId);
$idChat = $row['id'];

/* Agrega los participantes del chat */

$sql2 = "INSERT chatsParticipante VALUES ('$legajo', $idChat, '$nombre', 'Creador')";
$result = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sql2);
for ($i = 0; $i < count($elegido); $i++) {
    $sql = "SELECT nombre FROM [usuario] WHERE legajo = '$elegido[$i]'";
    $result = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sql);
    $row = sqlsrv_fetch_array($result);
    $nombre = $row['nombre'];

    $sql2 = "INSERT chatsParticipante VALUES ('$elegido[$i]', $idChat, '$nombre', 'Sin leer')";
    $result = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sql2);
}
...
function participantes() {
    global $idChat;
    $sqlParticipante = "SELECT * FROM [chatsParticipante] WHERE idChat = $idChat AND estado != 'Creador'";
...
function mensaje() {
    global $idChat;
    $sqlParticipante = "SELECT * FROM [chatsMensaje] WHERE idChat = $idChat";
    $result = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sqlParticipante);
    $html = '';
    if ($result) {
        $filas = sqlsrv_has_rows($result);
...
function titulo() {
    global $idChat;
    $sqlTitulo = "SELECT tema FROM [chats] WHERE id = $idChat";
    ...
    <head>
        ...
        <script>
            $(document).ready(function () {
                ...
                function mensaje() {
                    $('#mensajeTabla').load('mensaje.php', {id:<?php echo $idChat;?>});
                }
                setInterval(participan, 6000);
                function participan() {
                    $('#participantesTabla').load('participante.php', {id:<?php echo $idChat;?>});
                }
            });
        </script>
    </head>
                        ...
                        <form action="ingresarChat.php" method="post">
                            <div class="col" id="mensajeTabla">
                                <table id="mensaje" class="table table-striped table-bordered" border="3" style="width: 100%" style="margin-top: 0px; margin-bottom: 0px; height: 340px;">
                                    ...
                                    <tbody>
                                    <input type="hidden" id="chat" name="chat" value="<?= $idChat; ?>">
