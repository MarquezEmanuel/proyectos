...
$resultado = $script = "";
$queryPersonas = "select * from [dbo].[personasFisicasDuplicadas]";
$result = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $queryPersonas);
if ($result) {
    if (sqlsrv_has_rows($result)) {
        $resultado = "
        <div class='table-responsive mb-4'>
            <table id='tbRepPersonasFisicasDup' class='table table-striped'>
        ...
        $script = "
            <script>
                $(document).ready(function () {
                    $('#tbRepPersonasFisicasDup').DataTable({
                        dom: 'Bfrtip',
                        responsive: true,
                        pageLength: 20,
                        buttons: ['excel'],
        ...
        $resultado = '<div class="alert alert-warning text-center" role="alert"> No se encontraron personas físicas duplicadas </div>';
    }
} else {
    Log::escribirError("[Error al realizar la consulta de personas fisicas duplicadas][QUERY: $queryPersonas]");
    $resultado = '<div class="alert alert-danger text-center" role="alert"> Error al realizar consulta de personas físicas duplicadas</div>';
}
?>
<div class="container mt-4">
    <div id="superior">
        <h4 class="text-center mb-4 mt-4">PERSONAS FÍSICAS DUPLICADAS</h4>
