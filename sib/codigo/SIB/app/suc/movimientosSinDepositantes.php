...
function movimientosSinDepositantes() {
    ...
    $sql = "SELECT *,convert(varchar,cast(montoOrigen as money),1) AS montoOrigen2, convert(varchar,cast(montoPesos as money),1) AS montoPesos2 
	FROM [3movimientoSinDepositantes] WHERE fechaActualizacion between '{$actual}' AND '{$actualfinal}' AND codigoSucursal = {$_SESSION['sucursal']}";
