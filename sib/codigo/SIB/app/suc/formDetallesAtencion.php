...
<div class="container">
    <div id="contenido">
        ...
        AND OrganizationCode = {$idSucursal} AND DTECode NOT LIKE '%Totem%' AND 
        StateId = 4 AND AdminStopped = 0) b WHERE a.PartitionId = {$fecha} AND a.OrganizationCode = {$idSucursal} 
        AND a.DTECode NOT LIKE '%Totem%' AND a.StateId = 4 AND a.AdminStopped = 0";
        ...
        if (!$idSucursal || !$result) {
            ...
            PartitionId = {$fecha} AND OrganizationCode = {$idSucursal} AND DTECode NOT LIKE '%Totem%' AND StateId = 4 
            ...
            $consulta = "SELECT TOP 1 * FROM [VM000DB00].[STE].[dbo].[vw_H_INS_TaskInterval] WHERE 
            PartitionId = {$fecha} AND OrganizationCode = {$idSucursal} AND DTECode NOT LIKE '%Totem%' AND StateId = 4 
            AND AdminStopped = 0 ORDER BY DeadTime DESC";
            $resultado = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $consulta);
            $fila = sqlsrv_fetch_array($resultado, SQLSRV_FETCH_ASSOC);
            $fechaAlta = isset($fila['EndDate']) ? $fila['EndDate']->format('H:i:s') : "";
            $fechaFin = isset($fila['RealEndDate']) ? $fila['RealEndDate']->format('H:i:s') : "";
            $maximoMuerto = restar_horas($fechaAlta, $fechaFin);

            Log::escribirError("[Error al final del turnero][QUERY: $query]");
            Log::escribirError("[Error al final del turnero segunda consulta ][QUERY: $sql]");
            Log::escribirError("[Error al final del turnero tercera consulta ][QUERY: $consulta]");
            ?>
            ...
            <div id="centro" class="container">
                <div class="container">
                    ...
                    <div class="form-group row">
                        ...
                        <label for="nombreCliente" class="col-sm-2 col-form-label">Horas Totales de Atencion:</label>
                        <div class="col">
                        ...
                        <div class="col">
                            <input type="text" class="form-control mb-2" value="<?= $cantidad; ?>" readonly>
                        </div>
                        <label for="nombreUsuario" class="col-sm-2 col-form-label">Tiempo Medio de Atencion:</label>
                        <div class="col" >
                            <input type="text" class="form-control mb-2" value="<?= $hh . ":" . $mm . ":" . $ss; ?>" readonly>
                        </div>
                        <div class="w-100"></div>
                        <label for="fechaAlta" class="col-sm-2 col-form-label">Maximo Tiempo de Atencion:</label>
                        <div class="col">
                            <input type="text" class="form-control mb-2"  value="<?= $maximoEspera; ?>" readonly>
                        </div>
                        <label for="fechaAlta" class="col-sm-2 col-form-label">Usuario:</label>
                        <div class="col">
                            <input type="text" class="form-control mb-2"  value="<?= $row['UserCode']; ?>" readonly>
                        </div>
                        <div class="w-100"></div>
                        <label for="fechaAlta" class="col-sm-2 col-form-label">Puesto:</label>
                        <div class="col">
                            <input type="text" class="form-control mb-2"  value="<?= $row['DTECode']; ?>" readonly>
                        </div>
                        <div class="w-100"></div>
                        <label for="fechaAlta" class="col-sm-2 col-form-label">Mayor Tiempo Sin Atencion:</label>
                        <div class="col">
                            <input type="text" class="form-control mb-2"  value="<?= $maximoMuerto;
        } ?>" readonly>
                    </div>
                    <label for="fechaAlta" class="col-sm-2 col-form-label">Usuario:</label>
                    <div class="col">
                        <input type="text" class="form-control mb-2"  value="<?= $fila['UserCode']; ?>" readonly>
                    </div>
                    <div class="w-100"></div>
                    <label for="fechaAlta" class="col-sm-2 col-form-label">Puesto:</label>
                    <div class="col">
                        <input type="text" class="form-control mb-2"  value="<?= $fila['DTECode']; ?>" readonly>
                    </div>
