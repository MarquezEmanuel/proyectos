...
$querySolicitudes = "SELECT * FROM solicitudesWF ";
if ($sucursal != NULL) {
    $querySolicitudes .= " WHERE SUCURSAL = {$sucursal} ";
    if ($procesos != NULL) {
    ...
    if ($procesos != NULL) {
		$querySolicitudes = $querySolicitudes . "WHERE PROCESO IN(";
        for ($i = 0; $i < count($procesos); $i++) {
            $i = $i +1;
            if($i == count($procesos)){
                $i = $i -1;
        ...
        if($estados != NULL){
		$querySolicitudes = $querySolicitudes . "WHERE DESCRIPCION IN(";
        for ($i = 0; $i < count($estados); $i++) {
            $i = $i +1;
            if($i == count($estados)){
                $i = $i -1;
        ...
        if ($fechaInicio != NULL && $fechaFin != NULL) {
			...
			if ($fechaInicio != NULL && $fechaFin != NULL) {
            $querySolicitudes .= "WHERE CAST(FECHAESTADO AS DATE) >= CAST('{$fechaInicio}' AS DATE) AND CAST(FECHAESTADO AS DATE) <= CAST('{$fechaFin}' AS DATE)";
        }
		}
    }
}
