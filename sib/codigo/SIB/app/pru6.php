...
$grupos_permitidos = array('Gestion y Control de TI');
	...
	$objet->connect($ad_dc);
	$objet->setOptions();
	$objet->bind('SANTACRUZ\svpprod', 'FME168qu');	## DESCOMENTADO
     ...
     if ($entradas[0]['memberof']['count'] > 0)
		{
		  ...
		  $sucursal = explode ("," , $data);

          $buscar = "Suc";
          $temp = current($sucursal);
          $longitud = strlen($buscar);
           ...
           while($temp)
           {
             ...
             if(strtoupper($sub)==strtoupper($buscar))
               {
                 return substr($temp, -3);
               }
			     //$xx.="Cadena actual: $temp - SubString: $sub<br/>";
                 $temp=next($sucursal);
           }
          }
  	    }
    }		
	//$sucursal= $entradas[0]['memberof'];
	//return $sucursal;
	//return $entradas;
	//return $xx;
}
