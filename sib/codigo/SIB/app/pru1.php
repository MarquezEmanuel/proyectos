...
$grupos_permitidos = array('Gestion y Control de TI');
	...
	$xx->connect($ad_dc);
	$xx->setOptions();
	$xx->bind('SANTACRUZ\svpprod', 'FME168qu');
	
	
	$filtro = '(&(objectclass=person)(!(objectclass=contact))(!(objectclass=computer))(samaccountname='.$usr.'))';
	$atributos = array('displayname');//
	$xx->search($ad_dn, $filtro, $atributos);
	$entradas = $xx->get_entries();
	$nombre = 'nada';
	if ($entradas['count'] == 1)
	{
		if (!empty($entradas[0]['displayname'][0]))
		{
			$nombre = $entradas[0]['displayname'][0];
		}
	}
...
$obtieneNombre = obtenerNombre(quitarDominio($_SERVER['LOGON_USER'],'SANTACRUZ'));
...
<div class="form-group row">
                        <label for="causal" class="col-sm-2 col-form-label">Obtener Nombre:</label>
                        <div class="col">
							<?php echo $obtieneNombre;?>
                        </div>  
						</div>
