...
if ($legajo) {
    ...
    /* cambiar estado participantes del chat */

    if (!empty($_POST["espera"])) {
        $sql = "UPDATE [chatsParticipante] SET estado = 'Pendiente' WHERE legajo = '$legajo' AND idChat = $chat";
        $cambioEstado = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sql);
    } else {
        if (!empty($_POST["sinLeer"])) {
            $sql = "UPDATE [chatsParticipante] SET estado = 'Sin leer' WHERE legajo = '$legajo' AND idChat = $chat";
            $cambioEstado = sqlsrv_query(BDConexion::getInstancia()->getConexion(), $sql);
        }else{
            $cambioEstado = false;
        }
    ...
    if ($cambioEstado) {
        ...
        $mensaje = "<div class='alert alert-success text-center' role='alert'> Estado cambiado con exito <div>";
    } else {
        if (!$cambioEstado) {
            $mensaje = "<div class='alert alert-success text-center' role='alert'> No se pudo cambiar el estado solicitado, error en los estados o participantes <div>";
        }
    }
} else {
    $log = new Log();
    $log->writeLine("[No se pudo cambiar el estado del usuario porque no existe][ID: $legajo]");
